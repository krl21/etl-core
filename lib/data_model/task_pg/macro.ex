...
defmodule DataModel.TaskPg.Macro do
    @moduledoc """
    ...
    Each implementing module decides how to compose its own `insert_by_lote/3`.

    ## Available Macros

    ### Configuration Macros
    - `task_config/1` - Configure the task entity (app, table_name, group_by_keys, etc.)
    ...
    defmodule MyApp.TaskPg do
        ...
        def insert_by_lote(batch, batch_id, pg_config) do
            ...
            # - execute_insert/3 (creates temp connection internally)
    ...
    The `execute_insert/3` and `execute_insert_with_retry/3` functions now receive
    `pg_config` (connection configuration) instead of `pg_conn` (active connection).
    They create a temporary connection, perform the insert, and close the connection.
    All operations are wrapped in try-rescue with logging and Slack notifications.
    """
    ...
    @doc """
    Macro to configure the TaskPg entity.
    ...
    defmacro task_config(opts) do
        quote do
            ...
            def slack_webhook_url() do
                path = @task_config[:slack_webhook_url_path]
    ...
    @doc """
    Macro that generates helper functions (building blocks) for TaskPg entities.

    These are tools you can use to build your own `insert_by_lote/3`.
    ...
    ## Note
    A default `insert_by_lote/3` is generated but you can override it
    ...
    defmacro generate_task_helper_functions do
        quote do
            ...
            ### Returns:
                - `{:ok, record_map}` if successful
                - `{:error, nil}` if there's an error
            """
            def prepare_record(batch_id, composite_key, payloads) do
                try do
            ...
            defoverridable prepare_record: 3

            @doc """
            Executes the insert operation in PostgreSQL using insert_many.
            Creates a temporary connection, inserts, and closes the connection.

            ### Parameters:
                - `records`: List of record maps to insert
            ...
            ### Returns:
                - any
            """
            def after_insert(_batch, _batch_id, _pg_config), do: :ok
            defoverridable after_insert: 3
            ...
            @doc """
            Default implementation of insert_by_lote for PostgreSQL.
            ...
            def insert_by_lote(batch, batch_id, pg_config)
                ...
                result = execute_insert_with_retry(records, pg_config, batch_id)

                after_insert(batch, batch_id, pg_config)
