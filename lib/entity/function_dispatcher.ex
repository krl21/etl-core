defmodule Entity.FunctionDispatcher do
  @moduledoc """
  ...
  Checks if the entity has a custom function name defined, otherwise
  returns the default function name.

  ### Parameters:
      - `entity`: Module that implements Entity.Behaviour

  ### Return:
      - Atom representing the function name to call

  ### Example:
      function_name = get_insert_function_name(Entity.Record)
  ...
  This function will call either the standard `insert_by_lote/3` or
  the custom function name if `custom_insert_function_name/0` is implemented.
  ...
  ### Example:
      Entity.FunctionDispatcher.call_insert_function(
        Entity.Record,
        [%{"id" => "1"}],
        "batch-123",
        []
  ...
  def call_insert_function(entity, batch, batch_id, additional_info \\ []) do
    ...
    if function_exported?(entity, function_name, 3) do
      apply(entity, function_name, [batch, batch_id, additional_info])
    else
      raise """
